...
/**
* @brief Infinite loop that sounds out ICMP echo packets and processes replies
*
* Constructs echo packets and processes replies as they are recieved. If a response
* takes longer than the timeout value, a packet loss error is reported
*
* If IP address or hostname is found to be invalid an exception is thrown.
*/
  void start();
...
private:
   ...
   *
   * @return in_addr struct containing populated address fields containing address
   *
   * @throw std::invalid_argument if string representation cannot be converted to address
   */
  ...
  *
  * Because there is no assurance that ICMP replies will be recieved in the order
  * they were dispatched, responses are stored in a hash map with
  * the packet sequence acting as the key and a chrono timepoint as the value.
  * Upon the socket receiving a response, an elapsed duration is calculated
  * by comparing the timepoint value with that key and the timepoint representing
  * now.
  *
